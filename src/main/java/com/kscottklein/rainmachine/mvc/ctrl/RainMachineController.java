...
public class RainMachineController implements IZoneController, IProgramController {
	...
	public void configured(AuthResponseEntity response) {
		...
		this.setExpirationTimer(response.getExpires_in());

		try {
			ConfigManager.saveConfiguration(this.model.getConfiguration());
		} catch (IOException e) {
			RainMachineController.log.error("Error saving credentials", e);
		}
	}
	...
	public void getPrograms() throws RainMachineException {
		try {
			ProgramResponseEntity resp = RainMachineClient.getInstance()
					.programs(this.model.getAuthResponse().getAccess_token());
			this.setPrograms(resp);
		} catch (RestClientException e) {
			final String msg = "Error calling REST API for programs.";
			RainMachineController.log.error(msg, e);
	...
	public void getZones() throws RainMachineException {
		try {
			ZonesResponseEntity resp = RainMachineClient.getInstance()
					.zones(this.model.getAuthResponse().getAccess_token());
			this.setZones(resp);
		} catch (RestClientException e) {
	...
	public void loadConfiguration() {
		if (ConfigManager.configExists()) {
			try {
				...
				RainMachineController.log.error("Error loading configuration", e);
			}
		}
	}
	...
	private void setExpirationTimer(long expiresIn) {
		...
		timer.schedule(new TimerTask() {
			...
			public void run() {
				try {
					RainMachineController.this.authenticate();
				} catch (RestClientException e) {
					RainMachineController.log.error("Error re-authenticating", e);
				}
			}
		}, expiresIn);
		...
		RainMachineController.log.info("Token will expire at: " + expirationTime);

	}

	private void setPrograms(ProgramResponseEntity resp) {
		this.model.setPrograms(resp);
		this.listenersPrograms.stream().forEach(l -> l.programsUpdated());
	}

	private void setZones(ZonesResponseEntity resp) {
		this.model.setZones(resp);
		this.listenersZones.stream().forEach(l -> l.zonesUpdated());
	}

	@Override
	public void startZoneTimer(int id, int seconds) {
		final String accessToken = this.model.getAuthResponse().getAccess_token();
		final String state = String.format("Start Zone [%d] for [%d] seconds", id, seconds);
		RainMachineController.log.debug(state);
		...
		TimerTask tt = new TimerTask() {
			...
			public void run() {
				try {
					...
					RainMachineController.log.error("Error Stopping Zone: {}", state);
				}
	...
	public void stopZone(int id) {
		RainMachineController.log.debug("Stopping zone [%d], id");
		final String accessToken = this.model.getAuthResponse().getAccess_token();
		RainMachineClient.getInstance().startStopZone(accessToken, id, false);
	}
